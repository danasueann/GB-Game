...
// choose_name.c - Implementation file
...
UINT8 char_to_tile(char c)
{
    ...
    {
        return FONT_TILE_OFFSET + 26 + (c - '0'); // Assuming numbers follow letters
    }
    else if (c == ' ')
    {
        return FONT_TILE_OFFSET + 36; // Space character
    }
    else if (c == ':')
    {
        return FONT_TILE_OFFSET + 37; // Colon
    }
    else if (c == '_')
    {
        return FONT_TILE_OFFSET + 38; // Underscore
    }
    else if (c == '[')
    {
        return FONT_TILE_OFFSET + 39; // Left bracket
    }
    else if (c == ']')
    {
        return FONT_TILE_OFFSET + 40; // Right bracket
    }
...
void show_message(const char *message)
{
    ...
    UINT8 start_x = (20 - msg_len) / 2; // Center on 20-character screen
    draw_text_at(start_x, 9, message);
}

void clear_screen(void)
{
    // Fill background with space tiles
